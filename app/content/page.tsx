...
export default function ContentSprintPage() {
  ...
  const tabBtn = (key: "pipeline" | "scripts", label: string): React.CSSProperties => ({
      ...
      <main style={{ maxWidth: 1120, margin: "0 auto", padding: "28px 24px 64px" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 20, flexWrap: "wrap", marginBottom: 20 }}>
          <div>
            ...
            <h1 style={{ margin: "4px 0 2px", fontSize: 26, fontWeight: 700, letterSpacing: "-0.02em", color: "var(--ink)" }}>{total} видео за спринт</h1>
        ...
        <div style={{ display: "flex", gap: 4, borderBottom: "1px solid var(--border)", marginBottom: 24 }}>
          <button style={tabBtn("pipeline", "Пайплайн")} onClick={() => setTab("pipeline")}>Пайплайн</button>
          <button style={tabBtn("scripts", "Сценарии")} onClick={() => setTab("scripts")}>Сценарии</button>
        </div>
              ...
              {sel ? (
                <>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 4 }}>
                    <span style={{ fontSize: 13, color: "var(--muted)" }}>Ролик {sel.n} · автор {sel.author}</span>
                  </div>
